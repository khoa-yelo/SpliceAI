    ...
    with gzip.open(gff_path, 'rt') as fh:
        ...
        for tx_id, meta in transcripts.items():
            if meta['parent'] in transcripts_unique:
                if len(meta['exons']) > len(transcripts_unique[meta['parent']]['exons']):
    ...
    for tx_id, meta in tqdm(transcripts.items()):
        ...
        for s,e in exons:
            ...
            if (d == L-1) or (a == 0) or (s == e):
                continue
            assert a != d, f"Donor and acceptor at same position {a} {d} for {tx_id},\
                  {donor_idxs}, {acceptor_idxs}, {exons}, {strand}, {tx_s}, {tx_e}, {chromosome_map[chrom]}, {s}, {e}"
            assert 0 < a < L, f"Acceptor {a} out of bounds for {tx_id}, {chromosome_map[chrom]}, {s}, {e}, {strand}, {tx_s}, {tx_e}, {raw}"
...
def main():
    ...
    fasta_path = download_if_needed(args.fasta or 'GRCh38.p14_genomic.fna.gz', FASTA_URL)
    gff_path   = download_if_needed(args.gff   or 'GRCh38.p14_genomic.gff.gz',   GFF_URL)
    chrom_path = download_if_needed(args.chrom or 'GCF_000001405.40_GRCh38.p14_assembly_report.txt', CHROM_URL)
    ...
    logging.info(f"Encoding and blocking transcripts")
    all_recs = list(encode_and_block(transcripts, fasta, chromosome_map, flank=args.flank))
    ...
    # test 1, 3, 5, 7, 9
    train_chrs = set([str(i) for i in range(10, 23)]) | {'X', 'Y'} | set([str(i) for i in range(2, 10, 2)])
    ...
    val_tx = set(np.random.choice(list(train_tx), size=int(len(train_tx) * 0.1), replace=False))
    val = [r for r in train if r['tx_id'] in val_tx]
    train = [r for r in train if r['tx_id'] not in val_tx]

    logging.info(f"Train: {len(train)} blocks from {len(train_tx)} transcripts from {len(train_chrs)} chromosomes")
    logging.info(f"Val:   {len(val)} blocks from {len(val_tx)} transcripts from {len(train_chrs)} chromosomes")
    logging.info(f"Test:  {len(test)} blocks from {len(test_tx)} transcripts from {len(test_chrs)} chromosomes")
