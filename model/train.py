...
def get_dataloader(args):
    ...
    train_dataset = SpliceDataset(
        args.train_data, args.train_fraction, flank=args.flank
    )
...
def get_model(args):
    ...
    if args.flank == 5000:
        model = SpliceAI_10k(
            in_channels=4, mid_channels=32, out_channels=3, dropout=args.dropout
        )
...
def run_epoch(model, loader, criterion, optimizer, device, train=True):
    ...
    targets_onehot = F.one_hot(
        torch.tensor(targets), num_classes=probs.shape[1]
    ).numpy()
...
def train(args, logger):
    ...
    for epoch in range(args.num_epochs):
        ...
        for i, (ta, pa, ra, ca) in enumerate(
            zip(
        ...
        logger.info(
            ...
            train_metrics["true_counts"],
            train_metrics["pred_counts"],
        )
        logger.info(
            "  Val   counts (true → pred): %s → %s",
            val_metrics["true_counts"],
            val_metrics["pred_counts"],
        )
        logger.info("-" * 60)
