...
class SpliceDataset(Dataset):
    def __init__(self, path, frac, flank: int):
        ...
        if frac < 1.0:
            self.data = np.random.choice(self.data, int(len(self.data) * frac), replace=False)
        self.flank = flank
    ...
    def __getitem__(self, idx):
        ...
        if L < self.desired_len:
            raise ValueError(f"Sequence length {L} is shorter than desired {self.desired_len}")
