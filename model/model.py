"""
SpliceAI model with 10k, 2k, 400nt and 80nt receptive fields.
The model is based on the SpliceAI architecture described in
...
class SpliceAI_10k(nn.Module):
    """
    SpliceAI model with 10k receptive field.
    """
    ...
    def forward(self, x):
        ...
        # x = F.softmax(x, dim=1)  # since CrossEntropyLoss is used,
        # we don't need to apply softmax here
...
class SpliceAI_2k(nn.Module):
    """
    SpliceAI model with 2k receptive field.
    """
...
class SpliceAI_400nt(nn.Module):
    """
    SpliceAI model with 400nt receptive field.
    """
...
class SpliceAI_80nt(nn.Module):
    """
    SpliceAI model with 80nt receptive field.
    """
...
if __name__ == "__main__":
    ...
    model_10k = SpliceAI_10k(
        in_channels=4, mid_channels=32, out_channels=3, dropout=0.1
    ).to(device)
    model_2k = SpliceAI_2k(
        in_channels=4, mid_channels=32, out_channels=3, dropout=0.1
    ).to(device)
    model_400 = SpliceAI_400nt(
        in_channels=4, mid_channels=32, out_channels=3, dropout=0.1
    ).to(device)
    model_80 = SpliceAI_80nt(
        in_channels=4, mid_channels=32, out_channels=3, dropout=0.1
    ).to(device)
